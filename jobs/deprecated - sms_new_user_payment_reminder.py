...
carpark_id = os.getenv('NEW_CARPARK_ID')  # only send to monthly customers for new carparks
message_id = 2  # message id for first time customer payment reminder

# get the content for payment reminder SMS
sql = text("""
...
sql = text("""
    ...
    INNER JOIN carpark_config ON carpark_config.carpark_id = :carpark_id AND carpark_config.enable_monthly_rental = 1
    ...
    WHERE octopus.id IS NOT NULL AND vehicle.id IS NOT NULL AND user_message_log.id IS NULL AND mobile.is_verified = 1 AND mobile.date_deleted IS NULL;
...
for result in results:
    ...
    if response.content.decode("utf-8") == "-100":  # payment required
        ...
        print("One Way SMS API responded with invalid mobile number: " + str(country_code) + str(mobile_no))
        continue
...
#     "senderid": "Uvision",
#     "languagetype": 2,
#     "mobileno": f"85254037797",
#     "message": f"{sms_count} payment reminder SMSes sent\n\nContent: {msg['content']}"
# }
#
# response = requests.get("https://sgateway.onewaysms.com/apichinese20.aspx", params=params)
